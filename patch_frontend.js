...
const adminDashboardStart = code.indexOf('function AdminDashboard({ user }) {');
const appExportStart = code.indexOf('export default function App() {');

// We are going to replace AdminDashboard and AdminConfig, and add AdminUMS.
// Then we'll update the renderPage function and nav bar inside App.
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <p className="text-sm text-gray-500 mb-6">Values updated here instantly reflect across all executive devices for P & I math.</p>
              ...
              {config.incentives.map((product) => (
                <div key={product.id} className="flex items-center gap-4 bg-white p-3 rounded-lg border border-gray-200">
                  ...
                  <button type="button" onClick={() => removeProduct(product.id)} className="p-1 text-red-500 hover:bg-red-50 rounded">
                    <Square size={16} /> {/* Placeholder for a delete icon */}
                  </button>
                </div>
              ))}
...
function AdminDashboard({ user }) {
  ...
  const [inspectorMode, setInspectorMode] = useState('live'); // 'live' or 'history'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Payment Collection Report Card */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
      ...
      {/* Field Inspector (Live GPS Tracking) */}
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              {!selectedExec ? (
                ...
                <div className="bg-blue-50 rounded-xl border border-blue-100 p-6 h-full min-h-[300px] flex flex-col justify-center items-center relative overflow-hidden">
                   ...
                   {data.liveLocation?.find(l => l.id === selectedExec.id) ? (
                     ...
                     <p className="text-gray-500">Location data not available (Executive may be offline).</p>
                   )}
                </div>
              ) : (
...
code = code.replace(/function AdminDashboard\(\{ user \}\) \{[\s\S]*?function AdminConfig\(\{ user \}\) \{[\s\S]*?\n\n/m, newAdminCode + '\n\n');

// 2. We must completely replace the old AdminDashboard and AdminConfig logic in case the regex missed.
// Actually, it's safer to extract everything before AdminDashboard and everything after AdminConfig, and inject our new code.

const startIdx = code.indexOf('function AdminDashboard({ user }) {');
const endIdx = code.indexOf('function ProfileSettings({ user, onLogout }) {');
code = code.substring(0, startIdx) + newAdminCode + code.substring(endIdx);


// 3. Update Nav Bar inside App component to include UMS tab
const renderPageRegex = /const renderPage = \(\) => \{[\s\S]*?if \(isAdmin\) \{[\s\S]*?switch \(currentPage\) \{[\s\S]*?case 'admin-dashboard': return <AdminDashboard user=\{user\} \/>;\n\s+case 'admin-config': return <AdminConfig user=\{user\} \/>;[\s\S]*?\n\s+case 'profile': return <ProfileSettings user=\{user\} onLogout=\{handleLogout\} \/>;\n\s+default: return <AdminDashboard user=\{user\} \/>;\n\s+\}/;

const newRenderPage = `const renderPage = () => {
    if (isAdmin) {
      switch (currentPage) {
...
code = code.replace(renderPageRegex, newRenderPage);


const adminNavRegex = /<nav className="fixed bottom-0 w-full max-w-4xl mx-auto bg-white border-t border-gray-200 grid grid-cols-3 p-2 z-10 shadow-\[0_-4px_6px_-1px_rgb\(0,0,0,0\.05\)\]">[\s\S]*?<\/nav>/;

const newAdminNav = `<nav className="fixed bottom-0 w-full max-w-4xl mx-auto bg-white border-t border-gray-200 grid grid-cols-4 p-2 z-10 shadow-[0_-4px_6px_-1px_rgb(0,0,0,0.05)]">
          <button 
          ...
          >
            <Settings2 size={20} />
            <span className="text-[10px] font-medium truncate w-full text-center">Config</span>
          </button>
          <button 
          ...
          >
            <Users size={20} />
            <span className="text-[10px] font-medium truncate w-full text-center">UMS</span>
          </button>
          <button 
...
code = code.replace(adminNavRegex, newAdminNav);
